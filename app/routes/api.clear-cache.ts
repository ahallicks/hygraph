...
export async function action({
	...
	if (formData.data.__typename === 'Homepage') {
		...
		if (cachedResult) {
			...
			cache.printStats();
			return new Response(
				`Cache cleared for homepage: ${homepageCacheKey}`,
				{ status: 200 },
			);
		}
	...
	if (formData.data.__typename === 'Page') {
		...
		if (pagedCachedResult) {
			...
			cache.printStats();
			return new Response(`Cache cleared for page: ${pageCacheKey}`, {
				status: 200,
			});
		}
		...
		if (formData.operation === 'create') {
			...
			cache.printStats();
			return new Response(`Pages cache deleted: ${pagesCacheKey}`, {
				status: 200,
			});
		}
		cache.printStats();
		return new Response(`No cache found for page: ${pageCacheKey}`, {
			status: 200,
		});
	}
	if (formData.data.__typename === 'Header') {
		...
		if (headerCachedResult) {
			...
			cache.printStats();
			return new Response(`Cache cleared for header: ${headerCacheKey}`, {
				status: 200,
			});
		}
		cache.printStats();
	...
	console.log(
		'No matching typename for cache invalidation. Invalidating entire cache as fallback.',
	);
	cache.clear();
							...
							features: [
								{
									...
									title: 'Push to deploy.',
									updatedAt:
										'2026-01-20T16:59:40.55324+00:00',
								},
								{
									__typename: 'Feature',
									description:
